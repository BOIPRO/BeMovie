...
import { ConnectModel, Animes, Episodes } from './db.mjs';
...
const expandEpisodeList = async (page) => {
    try {
        const clicked = await page.evaluate(() => {
            // Tìm tất cả các thẻ a tập phim
            const candidates = Array.from(document.querySelectorAll('.latest_eps a[href*="/tap-"]'));
            if (!candidates.length) return false;

            // Click vào tập đầu tiên tìm được
            candidates[0].click();
            return true;
        });
        if (clicked) {
            console.log('Đã click trúng tập phim! Đang đợi trang tập phim mới tải xong...');

            try {
                // 2. Thay vì dùng delay(2000) hên xui, hãy ép Puppeteer đợi cho đến khi 
                // trang web chuyển hướng sang URL mới thành công (chỉ cần nhận bytes đầu tiên - commit)
                await page.waitForNavigation({
                    waitUntil: 'domcontentloaded',
                    timeout: 15000 // Tối đa 15 giây, nếu mạng Render lag quá thì bỏ qua
                });

            } catch (navError) {
                console.warn('Cảnh báo: Đợi chuyển hướng trang bị timeout hoặc đứt frame nhẹ, nhưng vẫn tiếp tục thử cào...');
                // Đôi khi trang phim chuyển hướng quá dị làm waitForNavigation báo lỗi, 
                // ta catch nó lại và cứ cho delay chủ động 3 giây rồi chạy tiếp để tránh sập bot.
                await new Promise(resolve => setTimeout(resolve, 3000));
            }
        } else {
            console.error('Không tìm thấy tập phim nào với selector hiện tại!');
        }
    } catch (e) {
...
const getServerFromToken = async (animevietsubUrl, serverDu, serverEmbed) => {
    ...
    try {
        ...
        if (serverDu) {
            try {
                ...
                const responseDu = await axios.post(API_URL, payloadDu, {
                    headers: ajaxHeaders,
                    transformRequest: transformFn
                });
                duData = responseDu.data || null;
            } catch (errDu) {
                console.error("Lỗi khi fetch riêng server DU:", errDu.message);
            }
        } else {
            console.log("Không có cấu hình server DU, bỏ qua fetch DU.");
        }
        await delay(500)
        if (serverEmbed) {
            try {
...
const getSeverStreamAniemVietSub = async (animevietsubUrl, episodeId) => {
    ...
    try {
        ...
        if (!serverDU && !serverEmbed) {
            throw new NotFoundException("Không tìm thấy cả server DU lẫn Embed trong danh sách.");
        }
        return await getServerFromToken(animevietsubUrl, serverDU, serverEmbed);
    }
    catch (error) {
        if (error.response) {
            console.error(`Lỗi từ phía Server (${error.response.status}):`, error.response.data);
...
const handleAsync = async (config) => {
    try {
        await ConnectModel(process.env.MONGOOSE_URI)
        ...
        if (process.send) {
            process.send({ success: true, type: 'DONE' });
        }
